...
<TypeTable
  type={{
    ...
    streamSegmentMaxAgeMs: {
      ...
      default: "240000",
      description:
        "Finalize and continue native streams after this many milliseconds to stay below Slack's stream expiry.",
    },
    feedbackButtons: {
...
Native streams are finalized and continued in a new message after four minutes by default, before Slack's approximately five-minute stream expiry. Open code fences are closed and reopened across segments. Set `streamSegmentMaxAgeMs` to adjust the rollover threshold.
